...
static uint32_t rb_dma_recved_len(void)
{
    return g_recvedLen;
}
...
DWORD WINAPI producer_thread(LPVOID lpParam) {
    ...
    while (local_produced < TOTAL_DATA_SIZE) {
        ...
        // Generate data
        generate_test_data(data_buffer, g_blocksize, local_pattern);

        // Config and start dma
        status = RingBufferDMAConfig(&g_rb, (RB_ADDRESS)data_buffer, CHUNK_SIZE);
        if (status) {
            printf("Producer: Config dma fail(%d)\n", status);
            continue;
        }
        status = RingBufferDMAStart(&g_rb);
        if (status) {
            printf("Producer: Start dma fail(%d)\n", status);
            continue;
        }

        uint32_t to_write = g_blocksize;
        uint8_t *to_write_buffer = (uint8_t *)g_detAddr;

        // Try to write to buffer in a lock-free manner
        uint32_t idx = 0;
        do {
            to_write_buffer[idx] = data_buffer[idx];
            idx++;
            local_produced++;
            g_recvedLen++;
        } while (--to_write);

        local_pattern += g_blocksize;
        MEMORY_BARRIER();
        ...
        do {
            ...
            if (status) {
                printf("Producer: Complete dma fail(%d)\n", status);
                printf("Producer: Retry...\n");
                SwitchToThread();
            } else {
                g_recvedLen = 0;
                break;
...
int main() {
    ...
    // Register ring buffer to be dma mode
    printf("Registering ring buffer to be dma mode...\n");
    result = RingBufferDMADeviceRegister(&g_rb, rb_dma_config, NULL, NULL, rb_dma_recved_len, NULL, NULL);
    if (result != RB_OK) {
        printf("Failed to register ring buffer: %d\n", result);
        return 1;
    }
    printf("Ring buffer register successfully, size: %u\n", RingBufferSizeGet(&g_rb));
